...
<!--  必须的手动指定高度，不然高度就是0就显示不出来-->
  <div id="chengdu_map" style="margin: 50px auto;width: 1000px;height: 800px"></div>
</template>

<script>
  import axios from "axios"
  import chengdu from "../../static/echarts-map-data-master/china-city/geojson/510100.json"
  export default {
    name: 'EchartsMap',
    data() {
      return{
    ...
    mounted () {
      const _this = this
      axios.get("../../static/map-data/chengdu.json").then(response => {
        console.log(response)
        _this.canvaschengduMap(response.data)
      })

    },
    methods: {
      //绘制中国地图,传入鼠标移入时显示的省份数据
      canvaschengduMap: function (mapDta) {
        // 基于准备好的dom，初始化echarts实例
        var myChart = this.$echarts.init(document.getElementById("chengdu_map"));
        myChart.showLoading();
          ...
          series: [
            {
              ...
              type: 'map',    //指定是地图类型
              map: 'chengdu',   //和上面registerMap中的一直
